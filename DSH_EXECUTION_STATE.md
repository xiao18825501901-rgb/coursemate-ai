...
**审批通道已核验可用（`ask`）**：本地收尾轮的每次仓库写入都经真实批准回执。
**仍未被批准**：真实千问付费调用、生产访问、数据库迁移、重启服务、修改真实凭证、
Git push、Netlify 发布。

已执行且不需要授权：本地读文件、本地改代码（经逐次批准）、本地非计费测试、本地构建、
本地浏览器测试。

> `scripts/Request-DeploymentApproval.ps1` 只产生业务同意记录，不接入
> Harness 审批决策（closure prompt §9 已区分四项，见最终报告 §7.1）。

## 5.1 阻塞与解除路径

**阻塞项**：真实千问两阶段 canary、生产部署 + 双用户验收——审批通道已可用（`ask`
并已验证），但这些动作的**具体批准（预算/部署范围）尚未发出**。

**解除阻塞所需的最小动作**（用户侧）：
批准 canary 的整批总预算（建议 CNY 5.00，先按实时价格估算总费用；上线持续费用另批），
随后按 `LAST_MILE_STATUS.md` 顺序逐项批准生产只读核验、备份/恢复演练与受控部署。
