...
export default defineConfig({
  ...
  webServer: [
    ...
    { command: `"${python}" -m uvicorn app.main:create_app --factory --host 127.0.0.1 --port 18101`,
      cwd: path.join(root, "services/rag-api"),
      env: { ...process.env, APP_ENV: "test", RAG_PROVIDER_MODE: "deterministic", V3_ENABLED: "true",
        AUTH_TEST_USER_ID: "v3-e2e-second", ADMIN_USER_IDS: "", WEB_ORIGIN: "http://127.0.0.1:15173",
        RAG_DATABASE_PATH: path.join(data, "rag.db"), RAG_UPLOAD_DIR: path.join(data, "uploads") },
      url: "http://127.0.0.1:18101/health", reuseExistingServer: false, timeout: 60000 },
    { command: `"${python}" -m uvicorn app.main:create_app --factory --host 127.0.0.1 --port 18102`,
      cwd: path.join(root, "services/rag-api"),
      env: { ...process.env, APP_ENV: "test", RAG_PROVIDER_MODE: "deterministic", V3_ENABLED: "true",
        AUTH_TEST_USER_ID: "v3-e2e-admin", ADMIN_USER_IDS: "v3-e2e-admin", WEB_ORIGIN: "http://127.0.0.1:15173",
        RAG_DATABASE_PATH: path.join(data, "rag.db"), RAG_UPLOAD_DIR: path.join(data, "uploads") },
      url: "http://127.0.0.1:18102/health", reuseExistingServer: false, timeout: 60000 },
