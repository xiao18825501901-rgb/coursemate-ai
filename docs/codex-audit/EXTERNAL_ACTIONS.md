# External actions — not executed or authorized by this audit

1. **Real model:** approve exact provider/account/region/model capability test and
   spending cap. Use normal (one work call) and Thinking (plan then work) canaries,
   exercises/reveal/explanations/cancellation; record provider receipts and actual
   cost privately. Local test labels are not live-model acceptance.
2. **Registered directory:** approve a protected Clerk full snapshot. Client code
   and paginated fake HTTP contracts exist in `cm_update/directory.py`; they have
   not contacted Clerk. CLI requires `--database` and `--approved-clerk-sync` plus
   backend-only CLERK_SECRET_KEY. Do not paste the secret into a command argument.
   Inspect counts/exclusions before applying to production.
3. **Existing-user qualification:** separately approve `--approved-grandfather`
   and trusted fixed `--cutoff-ms`. Missing/invalid creation timestamps are not
   silently eligible; disabled/deleted/banned records are excluded. Snapshot must
   be complete; sync alone does not grant qualification. Back up first and inspect
   the completed receipt, not just startup success.
4. **Migration/runtime:** first resolve and rehearse Windows publication failure
   or execute the unchanged acceptance suite on the actual supported Linux
   staging platform. See MIGRATION_AND_ROLLBACK.md. No production paths or hosts
   were accessed in this audit; current online version/provider are UNKNOWN.
5. **Messaging/sharing:** only synthetic local actors were used. Actual messages,
   shares, qualification changes and Clerk data are outside this authorization.
6. **Release:** no Git push, production deploy or public smoke test. Require a
   separate go-ahead, verified host/version, recovery unit, staged migration and
   rollback rehearsal. Do not label this local candidate PRODUCTION ACCEPTED.

Historical indexed vectors do not contain independently recorded provider/model
provenance. Snapshot export labels the configured embedding model and rejects a
different label on import; this is not proof that every legacy vector was created
by that configuration. Before production rollout audit historical index lineage;
unknown/incompatible indexes need a separately approved rebuild, never a hidden
paid embedding call during sharing.
