...
### 6.1 覆盖评审器（如启用）是收费的第三次调用，未授权不得运行

新壳自由文本教学的覆盖认定由可注入的 `CoverageReviewer` 决定（见
`INTEGRATION_MAP.md` 覆盖权威章节）。当前实现：

* `NullCoverageReviewer`（生产默认）：不确认任何覆盖，节点如实保持未覆盖；
* `DeterministicCoverageReviewer`：本地/测试规则，生产被 `validate()` 拒绝；
* **模型评审器（付费）**：计划为每次完成的教学 run 追加**一次** qwen3.8-max
  调用（输入≈教学内容+Spec items，输出≈最多 500 token 的 item 确认列表），
  费用并入同一 canary 总预算；**尚未实现，未获授权不得运行**。
  未启用前，生产覆盖的语义确认缺口如实记录（受限候选版本），不用关键词
  规则冒充评审。
