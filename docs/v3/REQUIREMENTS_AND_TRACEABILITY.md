...
Version: Stage 7 implementation baseline, 2026-09-12
...
| R-MIG-01 | Additive migration on isolated copies; preserve V2 rows/files/index/history | SQLite backup + numbered migrations | 011–021 + rehearsal/subset scripts | two-run idempotency, hashes/counts, publication/Assessment/budget invariants, integrity/FK, isolated restore | PARTIAL; synthetic initialization through 21 passed; last real-data copy ends at 18; final two-DB/uploads restore pending |
...
1. Keep migrations 011–021 and the Stage 1–7 regression suites; never rewrite already applied history.
2. Stage 7 local scope is complete: preserve the eight-case four-major matrix, zero-retry canaries and atomic daily model budgets. Paid calls remain Owner-gated and are not part of the local PASS.
3. Keep formal question author/review UI, complete official source-draft authoring/import, full accessibility/two-user browser coverage, AUTO routing and human `NEEDS_REVIEW` finalization visible as residual gaps.
4. Stage 8: rerun migrations 019–021 on a new read-only real-data copy, validate coordinated RAG/Agent/uploads restore, complete final browser/security regression, and produce a precise deployment handoff if production access remains unavailable.
