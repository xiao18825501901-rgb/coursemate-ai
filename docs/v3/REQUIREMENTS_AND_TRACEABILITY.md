...
Version: Stage 5 implementation baseline, 2026-09-12
...
| R-MIG-01 | Additive migration on isolated copies; preserve V2 rows/files/index/history | SQLite backup + numbered migrations | 011–018 + rehearsal/subset scripts | two-run idempotency, hashes/counts, Assessment invariants, integrity/FK, isolated restore | PARTIAL; pre-test 1–15 copy to 18 passed; local DB incident recovery verified; full two-DB/uploads restore pending |
...
1. Keep migrations 011–018 and the Stage 1–5 regression suites; correct applied history only with 019+ forward migrations.
2. Stage 6: add authorized official tree/Spec/question authoring, review, immutable publication snapshots and withdrawal without exposing private learning state.
3. Preserve Assessment/Teaching/Problem contracts while adding author/reviewer roles and validation transitions through new revisions, not mutable history.
4. Rerun the isolated real-data-copy rehearsal after every new migration and keep default browser tests on explicit `work/` copies.
