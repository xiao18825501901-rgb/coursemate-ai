...
Version: Stage 8 implementation baseline `7c6c54c`, 2026-09-12
...
|---|---|---|---|---|---|
| Q1 | `qwen3.8-max` is the primary intelligence model; Embedding is independent; backend owns auth, transactions and facts | Provider proposes structured output; API validates; DB commits | `config.py`, `learning/provider.py`, migration 021 call reservations, fixed synthetic canary, existing embedding adapter | T-Q1 exact model/endpoint lock, schema rejection, safe failed-call metadata, zero retry, budget preflight; live Chinese/image/stream/tool calls remain separate | SOURCE_IMPLEMENTED / LOCAL_CONTRACT_VERIFIED; live EXTERNAL_BLOCKED |
...
| Q9 | Teaching state machine + dynamic bounded Teaching Units; backend says what, model says how | Journey state, one bounded operation, explicit continue/replan/failure; reusable full plan; pre-call daily reservations | operation/unit runtime + `learning/plans.py` + migrations 015/021 | T-Q9 resume/idempotency/stale revision/invalid output/unknown outcome/cache; per-user and per-course daily quota | PARTIAL; implemented bounded explicit continuation and daily call gates, pause UI remains open |
| Q10 | Major → Course → Node Spec → User Adaptation; versioned REQUIRED/RECOMMENDED/OPTIONAL; official review | Versioned templates/specs; server compiler establishes rule precedence | `prompts/v3.2`, compiler, migrations 019/020, `knowledge_publication.py`, Stage 7 eight-case API/persistence matrix | T-Q10 four majors × case A/B; preferences cannot delete REQUIRED; exact independent review; unpublished changes invisible | SOURCE_IMPLEMENTED / LOCAL_FAKE_PROVIDER_VERIFIED; live quality and real human approval remain unverified |
...
| R-UI-01 | Two collaborative panes share state; horizontal/vertical, resizable; narrow screen preserves both | Server workspace state + React view | dual pane/range control, trees/axes and persistent Assessment panel | keyboard, screen-reader labels, 320px, refresh/relogin, race tests | PARTIAL; 375px Playwright, restore, visible control naming and basic keyboard focus verified; drag/tab/full screen-reader/axe/contrast audit pending |
...
| R-OPS-01 | No infinite autonomous loop; bounded operations, explicit continuation and cost controls | Operation state/idempotency, max 12-unit Plan, safe call ledger and atomic daily call reservations | orchestrator/provider + migrations 015/021; Agent minute/day limiter | concurrency, invalid/unknown outcome, no hidden retry, token/latency/usage ledger, per-owner/per-course quota, blocked-before-provider exclusion | SOURCE_IMPLEMENTED / LOCAL_CONTRACT_VERIFIED; production counters NOT_VERIFIED |
| R-MIG-01 | Additive migration on isolated copies; preserve V2 rows/files/index/history | SQLite backup + numbered migrations | 011–021 + rehearsal/subset scripts | two-run idempotency, hashes/counts, publication/Assessment/budget invariants, integrity/FK, isolated restore | PARTIAL; active RAG 1–18 → isolated 1–21 copy passed and source stayed unchanged; complete current two-DB/uploads restore BLOCKED by unknown authoritative Agent DB |
| R-DEPLOY-01 | Feature-flagged controlled release and rollback; production evidence is explicit | deploy config/runbook | V3 remains off; V3/Agent model intent, secret endpoint fields and quotas exist | V3-off/old frontend compatibility; preview; canary; rollback drill | PARTIAL; deploy intent only, runtime unknown |
...
| T-MODEL-02 | concurrent/daily over-budget call or missing local provider configuration | Reservation is atomic; over-limit never reaches Provider; explicit pre-provider `BLOCKED` does not consume paid-call quota |
| T-MIG-01 | V3 flag off on V2 database | No 011+ migration or V3-table query; V2 works through migration 10 |

## Current critical path

1. Keep migrations 011–021 and the Stage 1–8 regression suites; never rewrite already applied history.
2. Preserve the eight-case four-major matrix, zero-retry canaries, atomic daily model budgets, active-RAG-copy migration evidence and disjoint V2/V3 browser topologies. Paid calls remain Owner-gated and are not part of the local PASS.
3. Discover the authoritative current Agent DB path (or prove it is not deployed), then run the coordinated RAG/Agent/uploads backup and isolated restore before any preview or production mutation.
4. Keep formal question author/review UI, complete official source-draft authoring/import, full accessibility certification, AUTO routing, direct Problem contribution to coverage, integrated COMPOSITE exams and human `NEEDS_REVIEW` finalization visible as residual source/product gaps.
5. Owner-only next gates are the smallest live model canaries with human quality review, isolated preview, current production preflight, staged release and production smoke/rollback evidence.
