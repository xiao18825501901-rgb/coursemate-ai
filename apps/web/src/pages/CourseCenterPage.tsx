...
export function CourseCenterPage({ createMode = false }: { createMode?: boolean }) {
  ...
  useEffect(() => {
    ...
    void listCourses(getToken)
      .then(async (page) => {
        ...
        const counts = await Promise.all(page.items.map(async (course) => [
          course.id,
          (await listDocuments(getToken, course.id)).total,
        ] as const));
        if (active) setDocumentCounts(Object.fromEntries(counts));
      })
    ...
    return (
      <div className="page narrow-page">
        <header className="page-intro compact-intro">
          ...
          <h1>Create a private course</h1>
          <p>Your course, files, chunks, and conversations are visible only to you and administrators.</p>
        </header>
  ...
  const section = (title: string, items: Course[], empty: string) => (
    <section className="course-collection" aria-labelledby={`${title.replaceAll(" ", "-")}-title`}>
      <header><h2 id={`${title.replaceAll(" ", "-")}-title`}>{title}</h2><span>{items.length}</span></header>
      {items.length === 0 ? <div className="empty-panel">{empty}</div> : <div className="course-card-grid">{items.map((course) => (
        <article className="course-card" key={course.id}>
          <div className="course-card-topline"><span className="course-pill">{course.id}</span><span className={`visibility-badge visibility-${course.visibility ?? "public"}`}>{course.visibility === "private" ? "Private by default" : "Official"}</span></div>
          <h3>{course.name}</h3><p>{course.description || "No description yet."}</p><small>{documentCounts[course.id] ?? 0} documents</small>
  ...
  return <div className="page">
    ...
    {error && <div className="alert alert-error" role="alert">{error}</div>}
    {loading ? <div className="workspace-frame" aria-busy="true">Loading courses…</div> : <div className="course-collections">{section("Official Courses", official, "No official courses are available.")}{section("My Courses", mine, "Create your first private course to upload and teach from your own material.")}</div>}
  </div>;
}
