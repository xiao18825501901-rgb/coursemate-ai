...
import { request, send, remove, setTokenGetter, key } from './api.js';
...
export class App extends React.Component {
    state = { loading: true, error: '', config: null, user: null, courses: [], route: route(), drawer: null, modal: null, unread: 0, toast: '' };
    ...
    async authChanged() { try {
        ...
        if (user.id !== this.state.user?.id) {
            this.setState({ user, courses: [], drawer: null, modal: null, error: '', loading: false });
            await this.refresh();
        }
    }
    catch {
