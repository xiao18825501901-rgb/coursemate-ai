// M6F second-admin review driver v3 (round 4, same-window overlay).
// v2 put the review UI in a second tab, which ended up hidden behind the
// sign-in tab. v3 renders the review UI as a full-screen overlay INSIDE the
// same qqttai.com tab that holds the Clerk session (window.Clerk survives —
// the page is never navigated away). The overlay self-heals: if the React app
// re-renders and drops it, it is re-injected automatically. No deadlines:
// reviewer checks one box, then clicks explicit per-course approve/reject
// buttons. All API calls run in this same tab with the reviewer's own session.
...
const OUT = path.join(WORK, `m6f-second-admin-v3-${Date.now()}.json`);
...
// ---- build the overlay review UI (no <script>: events wired via evaluate) ----
...
<p style="color:#b91c1c;font-weight:bold">操作：先勾选页面下方的确认框，然后点击各门课程的“批准/拒绝”按钮。按钮为灰色时请先勾选确认框。</p>
...
<button id="m6fCsApprove" disabled style="background:#16a34a;color:#fff;font-size:16px;padding:10px 16px;margin:4px">CS3481 批准</button>
<button id="m6fCsReject" disabled style="background:#dc2626;color:#fff;font-size:16px;padding:10px 16px;margin:4px">CS3481 拒绝</button>
<button id="m6fGeApprove" disabled style="background:#16a34a;color:#fff;font-size:16px;padding:10px 16px;margin:4px 4px 4px 24px">GE2324 批准</button>
<button id="m6fGeReject" disabled style="background:#dc2626;color:#fff;font-size:16px;padding:10px 16px;margin:4px">GE2324 拒绝</button>
</div>
...
async function wireEvents() {
  await page.evaluate(() => {
    if (window.__m6fWired) return;
    const ack = document.getElementById("m6fAck");
    const buttons = ["m6fCsApprove", "m6fCsReject", "m6fGeApprove", "m6fGeReject"];
    const decided = {};
    ...
    document.getElementById("m6fCsApprove").addEventListener("click", function () { decide("cs", "approve"); });
    document.getElementById("m6fCsReject").addEventListener("click", function () { decide("cs", "reject"); });
    document.getElementById("m6fGeApprove").addEventListener("click", function () { decide("ge", "approve"); });
    document.getElementById("m6fGeReject").addEventListener("click", function () { decide("ge", "reject"); });
    window.__m6fWired = true;
  });
}

await injectOverlay();
await wireEvents();
console.log("OVERLAY_SHOWN — same window, no deadline; reviewer works at their own pace.");

// ---- wait for BOTH decisions (unlimited), re-injecting the overlay if the
//      React app re-renders and drops it ----
let decisions = null;
let reInjected = 0;
for (;;) {
  try {
    ...
    if (!state.overlay) { await injectOverlay(); reInjected += 1; console.log("OVERLAY_REINJECTED", reInjected); }
    if (!state.wired) { await wireEvents(); console.log("EVENTS_REWIRED"); }
  } catch (e) {
...
console.log("PHASE_COMPLETE=second-admin-review-v3");
await browser.close();
